...
紧凑 JSON 固定为 `{"classification_version":"2.0.0","as_of_utc":"UTC","posts":[{"id":"ID","at":"UTC","url":"X URL","text":"英文","zh":"中文","type":"TYPE","level":LEVEL,"reset_meaning":"MEANING","evidence":"分类依据","confirmed_event":false,"latest":true}],"sources":[{"id":"x","url":"来源URL","group":"x_original","scopes":["latest_overall"],"post_id":"顶部主帖ID"},{"id":"mirror","url":"独立来源URL","group":"search_engine","scopes":["latest_overall"],"post_id":"顶部主帖ID"},{"id":"official","url":"https://status.openai.com/","group":"openai_official","scopes":["official_status"]}],"status_indicator":"operational"}`。`posts[]` 必须补齐清单 `required_posts` 中缺失或原文不匹配的每个 ID，禁止只提交最大 ID；每条都必须显式填写 `zh/type/level/reset_meaning/evidence/confirmed_event`。`text` 逐字使用清单原文，脚本还会在合并后再次用清单恢复 ID、时间、URL 与原文，缺一条即拒绝写结果。只有清单 `latest_overall_post` 填 `latest:true`；已误收的普通回复用 `{"id":"ID","exclude":true}` 删除。

逐条按完整语义分类，不得按关键词命中：`reset_meaning:"none"` 表示没有 Codex 全局额度重置含义，使用非 `reset_signal` 类型、`level:0`、`confirmed_event:false`；`weak` 表示有语境相关但不明确的弱暗示，使用 `reset_signal/Lv.1/false`；`directional` 表示结合正文或同一帖子串可合理指向重置，使用 `reset_signal/Lv.2/false`；`explicit_future` 表示原文明说将执行重置并可附带落地窗口，使用 `reset_signal/Lv.3/false`；`completed` 只用于原文明确定义 Codex 全局额度已经重置，使用 `reset_signal/Lv.4`。同一已完成重置帖子串只有信息最完整的一条填 `confirmed_event:true`，其他已完成表述填 `false`。否定、玩笑、引用、征求反馈或仅出现 `reset` 一词不构成信号；即使上下文谈到重置，`surprise tomorrow` 之类没有明说惊喜就是重置的委婉暗示最多是 `directional/Lv.2`，不能升为明确承诺；`tomorrow`、`will`、`soon`、`landing in...` 等未来语义绝不能标为 Lv.4。`type` 只能是 `reset_signal`、`codex`、`limits`、`release`、`other`；`evidence` 必须说明整句语义和时态为何匹配该分类。`as_of_utc` 必须原样使用清单的 `checked_at_utc`。只提交缺失/不匹配/新增/修正帖子；默认省略 reasoning 和三个历史数组。`latest_overall` 来源必须各自使用不同 URL；`fetched_at` 只有来源正文明确给出时才填。若确实超过 3000 字节，分多次 `write_stdin`，只在最后一段加换行。
