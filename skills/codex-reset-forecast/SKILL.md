...
3. 两个来源至少应在以下任一项上一致：
   ...
   - 最新帖子发布时间；
   - 最新帖子正文的可唯一识别片段。
4. 若第二来源显示了更晚的帖子，必须先重抓并尝试取得更晚帖子的精确 ID、发布时间和正文；不得把已知较旧帖子声称为“最新”。
5. 若两个来源冲突且无法解决：
   - 不得声称“已经读取最新消息”；
   - 选择带精确帖子 ID、发布时间和可追溯 URL 的最新候选作为展示基准，并明确标记“未通过第二来源一致性核验”；
   - 只有同时具备精确 ID、发布时间、原文和可信分类的候选才能作为当前 Tibo 信号；只有搜索摘要、相对时间或缺失 ID 的更晚候选仅进入 LLM 证据解读，不得进入模型信号层；
   - 继续调用确定性脚本生成累计概率，固定令 `status = "degraded"`、模型状态为 `degraded`、结论置信度为 `low`，并把冲突写入 `explanation.limitations` 和 `source_consistency` 因素；
   - 若没有可审计的当前信号，使用 `baseline_fallback`，不得由 AI 根据摘要自行抬高或压低概率。
...
若已取得至少一个新鲜远程动态来源及另一个新鲜远程核验来源，但两者内容冲突，按 0.3 继续生成降级预测；内容冲突不等于无法联网。
...
- `抓取时间已确认且不超过20分钟`；
- `至少两个新鲜远程来源已完成抓取，即使内容存在已披露冲突`；
- `至少一个带精确 ID、发布时间和 URL 的最新总体动态候选可用于展示`；
- `最新重置相关动态已从可核验时间线中筛选；无法精确定位的更晚摘要未进入模型信号层`；
- `最近一次实际重置已核对`；
- `本次概率已丢弃旧结论并重新计算`。

全部来源一致时可按模型与回测结果输出 `ok`；存在未解决的来源时间或帖子 ID 冲突时必须输出 `degraded`，不得输出 `ok` 或 `high` 置信度。
