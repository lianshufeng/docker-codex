...
2. `RUN`：完整刷新禁止写输入文件。默认用带 TTY 的 `exec_command` 启动 `CODEX_RESET_STTY=$(stty -g 2>/dev/null || true); stty -echo -icanon min 1 time 0 2>/dev/null || true; IFS= read -r CODEX_RESET_DELTA; if [ -n "$CODEX_RESET_STTY" ]; then stty "$CODEX_RESET_STTY" 2>/dev/null || true; fi; export CODEX_RESET_DELTA; node /home/node/.agents/skills/codex-reset-forecast/scripts/forecast.mjs --input-env CODEX_RESET_DELTA --live-collection /tmp/codex-reset-live.json --print-report`；脚本会自动复用固定状态文件中的历史。取得 session ID 后用 `write_stdin` 分段发送单行紧凑 JSON，每段不超过 3000 UTF-8 字节且均不得包含换行；全部 JSON 发送完毕后，必须再单独调用一次 `write_stdin`，只发送一个换行符 `\n`。当外层运行明确禁用 TTY/交互（例如 Docker 的 `-T --interactive=false`）时，改用无交互等价入口：将同一份紧凑 JSON 作为环境变量直接传入 `CODEX_RESET_DELTA='...' node /home/node/.agents/skills/codex-reset-forecast/scripts/forecast.mjs --input-env CODEX_RESET_DELTA --live-collection /tmp/codex-reset-live.json --print-report`；仅做 shell 单引号转义，不得截断或改写 JSON，也不得等待 stdin。不要写输入文件，不用管道、heredoc、base64 或 `apply_patch`。
...
正常执行不调用 `--help`、`--smoke-test`、`--self-test`，不读取脚本，不回读完整 JSON。脚本返回 `blocked` 时允许写入不含概率的受阻状态 JSON 和配套页面，必须原样报告失败阶段、原因和产物路径并立即结束；若已有可用预测则保留旧产物。命令退出非零，或非 `blocked` 结果最终没有 `wrote_files:true`、`display_allowed:true`、`artifact_bundle_verified:true` 中任一条件时，必须原样报告失败阶段与原因并立即结束；禁止继续搜索 tracker 概率、禁止自行估算或展示任何概率。只有最终成功标记齐全时才可使用同一进程打印的 report 回答。
...
- 脚本固定生成并验证 JSON、HTML、本地服务器和启动入口；证据门禁阻断且没有可用旧预测时也生成不含概率的受阻状态产物。完整刷新原样返回 `--print-report`；快速复用只报告 ID 未变、结果时间和 JSON 路径，不复述内容。
